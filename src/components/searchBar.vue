<template>
    <div class="flex items-center gap-3 w-full app-window-padding">
        <input v-model="searchModel" class="flex-1 rounded-md bg-gray-200 border border-gray-600 text-center px-2 py-1 placeholder:text-gray-500" type="text" :placeholder="'Search '+props.search">
        <button @click="searchPosts" class="cursor-pointer flex items-center justify-center p-1 rounded bg-transparent transition-colors duration-150 hover:bg-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1f1f1f"><path d="M380-320q-109 0-184.5-75.5T120-580q0-109 75.5-184.5T380-840q109 0 184.5 75.5T640-580q0 44-14 83t-38 69l224 224q11 11 11 28t-11 28q-11 11-28 11t-28-11L532-372q-30 24-69 38t-83 14Zm0-80q75 0 127.5-52.5T560-580q0-75-52.5-127.5T380-760q-75 0-127.5 52.5T200-580q0 75 52.5 127.5T380-400Z"/></svg>
        </button>
    </div>
</template>

<script setup>
    import { ref } from 'vue';

    const emit = defineEmits(['search-query']);
    const props = defineProps({
        search:'',
    });

    const searchModel = ref('');

    function searchPosts(){
        const words = searchModel.value.toLowerCase().split(/\s+/);
        emit('search-query', words);
    }
</script>